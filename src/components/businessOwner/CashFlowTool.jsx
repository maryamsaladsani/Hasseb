...
function processCashFlowData(cashFlowData, dangerZone) {
    ...
    const processedData = cashFlowData.map((entry, index) => {
        ...
        if (index === 0 && entry.runningBalance) {
        ...
        const isWarning = runningBalance < dangerZone * 1.2 && runningBalance >= dangerZone;
    ...
    const lowestBalance = Math.min(...processedData.map(d => d.runningBalance));
    const currentBalance = processedData[processedData.length - 1]?.runningBalance || 0;
    const dangerMonths = processedData.filter(d => d.isDanger);
    const avgCashOut = processedData.reduce((sum, d) => sum + d.cashOut, 0) / processedData.length;
...
export default function CashFlowTool({ baseData }) {
    ...
    const handleDangerZoneChange = (e) => {
        setDangerZone(Number(e.target.value));
    };
            ...
            <div className="cashflow-top-section">
                ...
                <div className="danger-zone-section tooltip-wrapper">
                    <div className="danger-zone-header">
                        ...
                        <span className="danger-zone-value">{dangerZone.toLocaleString()} SAR</span>
                    </div>
                    <span className="tooltip-text">Haseeb lets you set your minimum comfortable cash balance. He'll warn you when your balance falls below this threshold so you can take action early.</span>
                    <input
                        type="range"
                        min="0"
                        max="50000"
                    ...
                    <div className="warning-banner-cashflow tooltip-wrapper">
                        ...
                        <div>
                            ...
                            <p className="warning-text-cashflow">
                                Your balance will fall below the danger zone in <strong>{result.summary.firstDangerMonth}</strong>.
                                Total danger months: <strong>{result.summary.dangerMonthsCount}</strong>
                            </p>
                        </div>
                        <span className="tooltip-text">Haseeb alerts you when your cash will drop below the danger zone you set. This gives you time to arrange financing or cut costs.</span>
                    </div>
                ) : (
                    <div></div>
                )}
            ...
            {result?.summary && (
                <div className="summary-cards">
                    <div className="summary-card tooltip-wrapper">
                        ...
                        <span className={`summary-value ${result.summary.currentBalance < dangerZone ? 'danger-value' : ''}`}>
                            {result.summary.currentBalance.toLocaleString()} SAR
                        </span>
                        <span className="tooltip-text">Haseeb shows you your latest cash position from your most recent transaction. This is where you stand right now financially.</span>
                    </div>
                    ...
                    <div className="summary-card tooltip-wrapper">
                        ...
                        <span className={`summary-value ${result.summary.lowestBalance < dangerZone ? 'danger-value' : ''}`}>
                            {result.summary.lowestBalance.toLocaleString()} SAR
                        </span>
                        <span className="tooltip-text">Haseeb warns you about the lowest point your cash will reach in the coming months. If this number is negative or below your comfort zone, you need to plan ahead.</span>
                    </div>
                    ...
                    <div className="summary-card tooltip-wrapper">
                        ...
                        <span className="tooltip-text">Haseeb calculates your average monthly spending. This helps you understand how much cash you typically use each month to run your business.</span>
                    </div>
                    ...
                    <div className="summary-card tooltip-wrapper">
                        ...
                        <span className={`summary-value ${result.summary.dangerMonthsCount > 0 ? 'danger-value' : 'safe-value'}`}>
                            {result.summary.dangerMonthsCount}
                        </span>
                        <span className="tooltip-text">Haseeb identifies how many months your balance will fall below your danger zone. Zero means you're safe throughout the period.</span>
                    </div>
                </div>
            )}
                        ...
                        <ResponsiveContainer width="100%" height={400}>
                            <LineChart data={result.data} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
                                ...
                                <ReferenceLine
                                    ...
                                    stroke="#FF5757"
                                    strokeDasharray="5 5"
                                    label={{ value: 'Danger Zone', position: 'right', fill: '#FF5757', fontSize: 12 }}
                                />
                                <Line
                                    type="monotone"
                                    dataKey="cashIn"
                        ...
                        <div className="table-wrapper">
                            <table className="cashflow-table">
                                ...
                                {result.data.map((row, index) => (
                                    <tr key={index} className={row.isDanger ? 'row-danger' : row.isWarning ? 'row-warning' : ''}>
                                        <td>{row.date}</td>
                                        <td>{row.description}</td>
                                        <td className="cash-in">{row.cashIn.toLocaleString()} SAR</td>
                                        <td className="cash-out">{row.cashOut.toLocaleString()} SAR</td>
                                        <td className={row.netCashFlow >= 0 ? 'positive' : 'negative'}>
